# STATUS - TODO 22 : Data & Recherche Android

**Progression** : 100%

## Phase 1 : Refonte UI Recherche (100%)

- [x] Cartouche description filtres actifs
- [x] Clic cartouche → déplie filtres
- [x] Recherche textuelle dans le panneau
- [x] Bouton "Appliquer"

## Phase 2 : Fix Filtre Grade (100%)

- [x] Corriger filtre niveau
- [x] Table FONT_GRADES avec IRCRA (`GradeUtils.kt`)
- [x] Logique epsilon (`getMaxIrcraForIndex`)

## Phase 3 : Filtre Setter Avancé (100%)

- [x] Mode Include/Exclude (`SetterFilterMode` enum)
- [x] UI ComboBox + checkboxes (`SetterFilterSection`)
- [x] Nombre de climbs par setter (`SetterInfo`)

## Phase 4 : Serveur & Sync (100%)

- [x] Fix dates `created_at` (1008 climbs corrigés)
- [x] Script SQL direct `fix_climb_dates_sql.py`
- [x] Endpoint bulk update `/api/climbs/bulk/dates`

## Phase 5 : Lazy Loading (100%)

- [x] Pagination infinie ClimbListScreen (`LazyListState` + `derivedStateOf`)
- [x] Chargement page suivante (`loadMore()` dans ViewModel)
- [x] Indicateur loading en bas de liste (`CircularProgressIndicator`)
- [x] Cache local Room (insertClimbs accumule les pages)
- [x] `PaginatedClimbs` data class avec `hasMore`
- [x] Page size réduit : 500 → 100 pour chargement progressif

## Phase 6 : Rendu Visuel (100%)

- [x] Lignes tapes START (serveur + Android)
- [x] Épaisseur tapes = contours (8px)
- [x] Dark mode + thème GRAY par défaut
- [x] Nouvelle icône app
- [x] Titre "Montoboard"

## Phase 7 : Parcours Santé 25 (Reportée)

- [x] Analyse spécificité : seulement 2 blocs sur 1017 (0.2%) utilisent le champ `circuit`
- [ ] ~~Fonctionnalité circuit~~ → Reportée (faible usage, nécessite modifications serveur + client)

**Note** : Le champ `circuit` contient un JSON de prises ordonnées. Exemple :
```json
[{"x": "1156.45", "y": 1722.64, "id": 829173, "value": "1"}, ...]
```

---

**Dernière mise à jour** : 2026-01-02
**Session** : TODO 22 complété (Phase 5 lazy loading, Phase 7 analysée et reportée)
